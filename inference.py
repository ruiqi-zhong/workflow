...
def sample_batched(
    ...
    for i, prompt in enumerate(prompts):
        return_dict[prompt] = [
            {'text': truncate_string(remove_prefix(all_completions[idx].replace(tokenizer.pad_token, ''), prompt), stop_strs), 
             'scores': [all_first_scores[idx][j] for j in save_score_tok_idx], 'orig': all_completions[idx]} 
