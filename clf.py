...
class T5ForSeqClassification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        starts = torch.tensor(
            [[self.model.config.decoder_start_token_id]] * len(input_ids)
        ).to(first_device_name)
        output = self.model(
            input_ids=input_ids, attention_mask=attention_mask, decoder_input_ids=starts
        )
...
class SeqClf(nn.Module):
    def __init__(self, pretrain_model, max_length=1024, load_path=None):
        super().__init__()
        self.pretrain_model = pretrain_model
        if "t5" in pretrain_model:
    ...
    def forward(self, data_dicts):
        ...
        if data_dicts[0].get("target") is not None:
            labels = torch.tensor(
                [[float(1 - d["target"]), float(d["target"])] for d in data_dicts]
            ).to(first_device_name)
    ...
    def evaluate_texts(self, texts, eval_bsize=32):
        ...
        with torch.no_grad():
            ...
            while cur_start < len(texts):
                input_dicts = [
                    {"input": t} for t in texts[cur_start : cur_start + eval_bsize]
                ]
                model_output_dict = self.forward(input_dicts)
                logits = (
                    lsm(model_output_dict["logits"].detach().cpu().float())
                    .numpy()
                    .tolist()
                )
            ...
            return {
                ...
                / (
                    1
                    + np.e ** (np.array(all_logits)[:, 0] - np.array(all_logits)[:, 1])
                ),
            }
    ...
    def evaluate_dicts(self, data_dicts, eval_bsize, scores_only=False):
        ...
        if not scores_only:
            saved_preds = [
                {"orig_d": data_dicts[i], "pred_score": scores[i]}
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                ...
                if not any(nd in n for nd in no_decay) and not frozen_param_name(n)
            ],
            "weight_decay": 0.01,
        },
        {
            "params": [
                p
                for n, p in model.named_parameters()
                if any(nd in n for nd in no_decay) and not frozen_param_name(n)
            ],
            "weight_decay": 0.0,
        },
    ]
    ...
    for mini_step in pbar:
        ...
        if save_path_prefix is not None and global_step_finishes_bool:
            ...
            if (
                ...
                and (global_step_finished % eval_every == 0)
                and global_step_finished > 0
            ):
                saved_preds = model.evaluate_dicts(all_data_dicts["eval"], eval_bsize)
                json.dump(
                    saved_preds,
                    open(
                        save_path_prefix + "-%d-preds.json" % global_step_finished, "w"
                    ),
                )

                if save_best_metric is not None:
                    result_dict = evaluate_pred_path(
                        save_path_prefix + "-%d-preds.json" % global_step_finished
                    )
                    if result_dict[save_best_metric] > best_performance:
                        best_performance = result_dict[save_best_metric]
                        print(
                            "At step %d, best performance is %.4f"
                            % (global_step_finished, best_performance)
                        )
...
if __name__ == "__main__":
    ...
    model = SeqClf(
        args.model_pretrain_name,
        load_path=args.model_init_path,
    ...
    if args.training_run_name is None:
        args.training_run_name = (
            args.data_name + "_" + args.model_pretrain_name.replace("/", "-")
        )
    ...
    if not os.path.exists(save_dir):
        ...
        print("Warning: save_dir already exists")
        exit(0)
    ...
    if not args.no_train:
        if args.ft_head_first:
            model = train_and_eval(
                ...
                num_steps=1000,
                save_path_prefix=None,
                train_bsize=args.train_batch_size,
                accumulate=args.gradient_accumulation_steps,
                save_every=None,
                eval_every=None,
                train_last_lyer_only=True,
            )
        resulting_model = train_and_eval(
            data_dicts,
            model,
            save_path_prefix=save_path_prefix,
            num_steps=args.max_steps,
            train_bsize=args.train_batch_size,
            eval_bsize=args.eval_batch_size,
            accumulate=args.gradient_accumulation_steps,
    ...
    preds = model.evaluate_dicts(
        data_dicts["eval"], args.eval_batch_size, scores_only=args.scores_only
    )
